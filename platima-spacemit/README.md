...
### Cross-build comparison (this fork vs Bianbu vs SpacemiT)

[`benchmarking/BENCHMARKS.md`](benchmarking/BENCHMARKS.md) holds the full three-way matrix —
**Bianbu 4.0.1 (0.1.1)** (the apt-shipped llama.cpp) vs **SpacemiT 0.1.3** (official release
tarball) vs **this fork** — across the Gemma 4 and Qwen3.5 families, `pp` and `tg`, FA on and
off, generated from [`results_bench.tsv`](benchmarking/results_bench.tsv) and
[`results_mtp.tsv`](benchmarking/results_mtp.tsv). Headlines:

- **Base throughput is identical within noise across all three builds** — the fork does not
  regress the SpacemiT A100 backend (e.g. `Qwen3.5-4B-Q4_K_M` 7.07 / 7.08 / **7.10** tg/s).
- **MTP is where the fork is unique.** Only this fork runs Gemma 4 MTP at all, and it roughly
  doubles Qwen MTP vs SpacemiT 0.1.3 (`Qwen3.5-4B` 4.70 → **8.40** tg/s). Best lifts:
  `gemma-4-12B-qat` **1.69×** (3.55 → 6.0) and `gemma-4-E4B-qat` **1.54×** (7.81 → 12.0).
- **Vanilla upstream is excluded** — upstream + SpacemiT flags heap-crashes on the
  prefill→generation transition; the SpacemiT backend carries an un-upstreamed fix.

## Patch history

See [`TODO.md`](TODO.md). Shipped patches: 1–14, 18 (Gemma4-assistant fit-probe log downgraded ERROR→DEBUG — the "MTP silently falls back" report was a misdiagnosis; MTP already works), 20 (Gemma 4 vision garbled-output fix — the custom IME2 transpose-cont kernel corrupts the vision encoder's F32 `ggml_cont(ggml_transpose(...))`; `GGML_OP_CONT` is now routed to generic CPU), 21 (vision encode faster, Tier 1 — bf16 mmproj weights re-typed to the vectorised F16 RVV path, ~24×), 22 (vision encode Tier 2 — bf16 weights quantized to q8_0 onto the IME2 int8 engine, a further ~1.9×; both tiers now auto-gated on CPU capability via the new `ggml_cpu_vec_dot_is_simd` predicate — see above), 23 (Gemma 4 12B Unified `gemma4uv` vision enabled via the upstream #24077 cherry-pick; tiny-image misread diagnosed as inherent RISC-V FP × model fragility, not a port bug — see above), 25 (mtmd video / frame-sequence support ported from #24269 — see above), 26 (drafter bf16 → q8_0/f16 retype at load, default-on; keeps a bf16 MTP head off the scalar path — see above), 27 (vision encode Tier 3 — F16 mmproj weights → q8_0 onto IME2, default-on; ~8–12% at native resolution, 199-image A/B found 0 accuracy regressions — see above), 28 (SpacemiT base 0.1.3 → 0.1.6 merge — cherry-picks #8 FunASR audio, #9 Qwen2-VL preproc, #10 LFM2 vision, #11 TCM barrier, #13 worker lanes, #15 MiniCPM-V RISC-V fix, #18 hybrid-recurrent multimodal; also adds graph-thread auto-clamp so `-t 8` is optional — see [`TODO.md`](TODO.md)), 29 (two new K3-validated multimodal models — DeepSeek-OCR-2 image OCR, fixed a clip-warmup crash by keeping the `resample_query` data tensor out of the IME2 retype; and Granite Speech 4.1 2B Plus audio, ported the multi-layer feature concat — see [`TODO.md`](TODO.md)), 30 (EAGLE3 speculative decode now loads and runs on K3 — fixed a `speculative-simple` segfault where `ctx_other` was only set for MTP spec types, generalising patch 7; adds a torch-free EAGLE3 → GGUF converter since riscv64 has no torch wheels. **Accept rate is still 0–1.4%, so EAGLE3 is not yet usable** — see [`TODO.md`](TODO.md)), 31 (SpacemiT 0.1.7 backend cherry-pick — `ggml-cpu/spacemit/` only, skipping the ONNX media/TTS wrappers we don't build; adds a `gemm_m == 1` **decode** fast path for q4_0/q8_0 plus RVV GELU/TANH/GEGLU. Measured A/B on `gemma-4-E2B_q4_0`: **tg 11.42 → 16.08 t/s (+40.8%)**, pp 116.39 → 136.80 (+17.5%); q4_K unchanged within noise — see [`TODO.md`](TODO.md)), 32 (spacemit `#30` q4_0 HP scale rescale — **reverted by patch 33, see below**; it also recorded that SpacemiT's active branch moved to `spacemit/mtmd-backend`, which `spacemit-mtmd` and the newest tag both lag), 33 (**revert patch 32** — `#30` silently destroyed speculative decoding: MTP accept on gemma-4-12B-qat went 64.5% → **0%** while throughput and text fluency both still looked fine. It rescales the HP sub-block and block scales inversely but leaves the q4_0 `a_sum` zero-point term unscaled. The overflow it fixes was reported against qwen2.5-3b q4_0, which we don't have and have never seen fail here. **Lesson: validate any kernel change with an MTP accept run, not t/s plus coherent-looking output** — see [`TODO.md`](TODO.md)). Deferred/dismissed: 15 (buffer-unification refactor — dismissed; see TODO), 16 (X100 sampling threadpool), 17 (trunk-graph probe — ROPE-RVV and Q4_1 HP-unlock both fail the ≥2%-of-decode gate), 19 (`llama-completion` spec args — the tool has no speculative loop). Document-only: 24 (tiny-image misread is spatial composition, not resolution — an auto-pad would distort non-text inputs; see TODO). Each entry records what was tried and why it was kept or dropped.
