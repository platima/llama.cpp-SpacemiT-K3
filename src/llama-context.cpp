...
llama_context::llama_context(
    ...
    if (model.arch == LLM_ARCH_EAGLE3) {
        if (model.tok_embd == nullptr || model.output == nullptr) {
            if (params.ctx_other == nullptr) {
                throw ctx_other_required_error("EAGLE3 requires ctx_other to be set (this is normal during memory fitting)");
            }
            cparams.ctx_other = params.ctx_other;
        }
    }
...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    // extract nextn (MTP) embeddings (post-output-norm hidden state)
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        // extract nextn (MTP) embeddings (post-output-norm hidden state for Gemma4 drafters)
        {
...
uint32_t llama_context::output_reserve(int32_t n_outputs) {
    ...
    const size_t new_size  =
        (logits.size + embd.size + embd_pre_norm.size + embd_nextn.size + embd_layer_inp_float_count + backend_float_count) * sizeof(float) +
