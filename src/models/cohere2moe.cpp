...
void llama_model_cohere2moe::load_arch_hparams(llama_model_loader & ml) {
    ...
    // This fork encodes the SWA pattern via hparams.swa_layers[] (populated by
    // set_swa_pattern) and reads it through hparams.is_swa(il); there is no
    // per-layer is_swa_impl array. Mirror cohere2.cpp: read the scalar period
    // and always seed the pattern (dense-first) so is_swa() is valid.
...
llama_model_cohere2moe::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    GGML_ASSERT(hparams.nextn_predict_layers > 0 && "COHERE2MOE MTP requires nextn_predict_layers > 0");
    GGML_ASSERT(hparams.nextn_predict_layers == 1 && "COHERE2MOE MTP currently only supports a single MTP block");

    const int64_t n_embd_head = hparams.n_embd_head_v();
    GGML_ASSERT(n_embd_head == hparams.n_embd_head_k());
    GGML_ASSERT(n_embd_head == n_rot);

    // MTP/NextN block occupies the first slot after the trunk:
    // il = n_layer - nextn_predict_layers (mirrors qwen35moe; no nextn_layer_offset here).
