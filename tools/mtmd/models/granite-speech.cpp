...
ggml_cgraph * clip_graph_granite_speech::build() {
    ...
    // granite-speech-plus: the projector consumes the concatenation of the selected
    // feature-layer outputs plus the final encoder output. proj_input_dim is
    // n_embd * (feature_layers + 1). Non-plus models have empty feature_layers, so
    // proj_input_dim == n_embd and the concat path is skipped.
    const int  proj_input_dim     = n_embd * ((int) hparams.feature_layers.size() + 1);
    ...
    // granite-speech-plus feature concatenation: capture layer 0 (encoder input)
    // if requested, then intermediate layers below, then the final output.
    ggml_tensor * concat_result = nullptr;
    if (use_feature_concat && hparams.is_feature_layer(0)) {
        concat_result = cur;
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        // granite-speech-plus: capture intermediate layer (il + 1) output if requested
        if (use_feature_concat && hparams.is_feature_layer(il + 1)) {
    ...
    // granite-speech-plus: append the final encoder output to the concatenated
    // features so proj input is [feature_layers..., final], matching proj_input_dim.
